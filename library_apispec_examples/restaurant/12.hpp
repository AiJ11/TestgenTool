...
/**
 * Restaurant Scenario 12: Delivery Agent Deliver Order
 * =====================================================
 * 
 * Flow: LoginOK(agentEmail) -> ViewAssignedOrdersOK -> UpdateOrderStatusOK(Order2, Delivered)
 * 
 * PREREQUISITE: Register a delivery_agent user first!
 */

class restaurant_example12 {
public:
    static void example(std::vector<std::unique_ptr<API>> &apis, SymbolTable &root) {
        ...
        // ========== API 3: UpdateOrderStatusOK(Order2, Delivered) ==========
        {
            // PRE: token_present(agentEmail)
            ...
            call3_args.push_back(std::make_unique<Var>("orderStatus"));
            auto call3 = std::make_unique<APIcall>(
                std::make_unique<FuncCall>("updateOrderStatus", std::move(call3_args)),
                Response(HTTPResponseCode::OK_200, nullptr)
            );

            // POST: order_status_updated(orderId, orderStatus)
            std::vector<std::unique_ptr<Expr>> post3_args;
            post3_args.push_back(std::make_unique<Var>("orderId"));
            post3_args.push_back(std::make_unique<Var>("orderStatus"));
            auto post3 = std::make_unique<FuncCall>("order_status_updated", std::move(post3_args));
            ...
            c3->symtable.insert(Var("orderStatus"));
