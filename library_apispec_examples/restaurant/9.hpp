...
/**
 * Restaurant Scenario 9: Owner View Orders and Assign
 * ====================================================
 * 
 * Flow: LoginOK(ownerEmail) -> ViewOrdersOK -> AssignOrderOK(Order2, AgentB)
 * 
 * PREREQUISITE: Run Scenario 6 first to register the owner!
 */

class restaurant_example9 {
public:
    static void example(std::vector<std::unique_ptr<API>> &apis, SymbolTable &root) {
        
        // ========== API 1: LoginOK(ownerEmail) ==========
        {
            // PRE: registered(ownerEmail) - owner exists in U
            ...
            call1_args.push_back(std::make_unique<Var>("password"));  // FIXED: Added password
            ...
            c1->symtable.insert(Var("password"));  // FIXED: Added password
        ...
        // ========== API 3: AssignOrderOK(Order2, AgentB) ==========
        {
            // PRE: token_present(ownerEmail)
