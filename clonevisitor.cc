...
unique_ptr<Expr> CloneVisitor::cloneExpr(const Expr* node) {
    ...
    switch (node->exprType) {
        ...
        case ExprType::INPUT:
            ...
            case ExprType::BOOL_CONST:
            return cloneBoolConst(dynamic_cast<const BoolConst&>(*node));
        case ExprType::BINARY_OP:
            return cloneBinaryOpExpr(dynamic_cast<const BinaryOpExpr&>(*node));
        case ExprType::UNARY_OP:
            return cloneUnaryOpExpr(dynamic_cast<const UnaryOpExpr&>(*node));
        default:
...
unique_ptr<Expr> CloneVisitor::cloneBoolConst(const BoolConst &node) {
    return make_unique<BoolConst>(node.value);
}

unique_ptr<Expr> CloneVisitor::cloneBinaryOpExpr(const BinaryOpExpr &node) {
    unique_ptr<Expr> clonedLeft = cloneExpr(node.left.get());
    unique_ptr<Expr> clonedRight = cloneExpr(node.right.get());
    return make_unique<BinaryOpExpr>(node.op, std::move(clonedLeft), std::move(clonedRight));
}

unique_ptr<Expr> CloneVisitor::cloneUnaryOpExpr(const UnaryOpExpr &node) {
    unique_ptr<Expr> clonedOperand = cloneExpr(node.operand.get());
    return make_unique<UnaryOpExpr>(node.op, std::move(clonedOperand));
}
